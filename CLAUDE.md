...
- **Repository test** — `CourtOrderRepositoryTest` (DeltaSpike `@RunWith(CdiTestRunner.class)` + embedded H2) → `CourtOrderRepositoryIT` using the real-Postgres `HibernateTestEntityManagerProvider` extension (JUnit 5 `@RegisterExtension`). It is gated behind the **`applicationscourtorders-integration-test`** Maven profile (failsafe) so it does NOT run in the DB-less PR/`mmse` build — only in the IT stage / locally via `-Papplicationscourtorders-integration-test`. Test persistence unit: `applicationscourtorders-test-persistence-unit` → `applicationscourtordersviewstore` DB.
